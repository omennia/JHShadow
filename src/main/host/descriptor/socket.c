...
void connect_socketToPeer(const char* hostname, const char* ip, in_port_t port) { // ip -> uint32_t, port -> uint16_t
    printf("[%lu] [Socket] connect socket to peer: %s, %s:%d\n", pthread_self(), hostname, ip, port);
}

void push_packet_socket(const char* hostname) {
    printf("[%lu] [Socket] push packet socket: %s\n", pthread_self(), hostname);
}

void add_to_output_buffer_socket(const char* hostname) {
    printf("[%lu] [Socket] add to output buffer socket: %s\n", pthread_self(), hostname);
}
...
void legacysocket_init(LegacySocket* socket, const Host* host, SocketFunctionTable* vtable,
    ...
    if (tracker != NULL) {
        CompatSocket compatSocket = compatsocket_fromLegacySocket(socket);
        //MODIFIED: Hugo Cardante & Joao Soares: Added const Host* host parameter
        tracker_addSocket(tracker, &compatSocket, socket->protocol, socket->inputBufferSize,
                          socket->outputBufferSize, host);
    }
}
...
gint legacysocket_connectToPeer(LegacySocket* socket, const Host* host, in_addr_t ip,
    ...
    //ADDED JSoares & HCardante
    connect_socketToPeer(host_getName(host), socket->peerString, port);
    //END
