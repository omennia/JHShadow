...
void new_tracker(const char *hostname, char *prt, char *msg) {
    printf("[Tracker] new tracker: %s\n", hostname);
    start_tracker(hostname);
}

void del_tracker(const char* hostname) {
    printf("[Tracker] del tracker: %s\n", hostname);
    end_tracker(hostname);
}

void add_socket(const char* hostname) {
    printf("[Tracker] add socket: %s\n", hostname);
    new_socket(hostname);
}

void send_data_socket(const char* hostname) {
    printf("[Tracker] add socket: %s\n", hostname);
    send_data(hostname);
}
...
Tracker* tracker_new(const Host* host, CSimulationTime interval, LogLevel loglevel,
    ...
    //ADDED: Hugo Cardante & Joao Soares
    tracker->hostname = host_getName(host);
    //END
...
void tracker_addSocket(Tracker* tracker, const CompatSocket* socket, ProtocolType type,
                       gsize inputBufferSize, gsize outputBufferSize, const Host* host) {
    MAGIC_ASSERT(tracker);
